...
def get_info(page):
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 YaBrowser/21.11.0.1996 Yowser/2.5 Safari/537.36'}
    ...
    for product in products:
        ...
        # запись данных в МонгоДБ
        products_base.insert_one(product_data)
...
db = client['products']
db.products.drop()
products_base = db.products_base
...
rating = int(input('Введите минимальный желаемый рейтинг(качество): '))

for product in products_base.find({'$or': [
    {'general_assessment': {'$gte': rating}},
    {'quality': {'$gte': rating}}
]}):
    pprint(product)
